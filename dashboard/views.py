...
def index(request):
    noQREventActive = QRevent.objects.filter(status='active').count()
    noQREventDisabled = QRevent.objects.filter(status='disabled').count()
    noOfRegistrations = QRForm.objects.all().count()
    return render(request, 'dashboard/index.html',{'noQREventActive':noQREventActive,'noQREventDisabled':noQREventDisabled,'noOfRegistrations':noOfRegistrations})
...
def qreventmanager_dwn(request, id):
    qr = QRevent.objects.get(pk=id)
    s = 'http://127.0.0.1:8000/dashboard/client/' + str(qr.id)
